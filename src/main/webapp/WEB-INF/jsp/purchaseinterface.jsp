...
<body>
	...
	<div style="position:absolute; width: 100%;height: 100%;left: 0px;top: 0px;background-image: linear-gradient(to top, #e6e9f0 0%, #eef1f5 100%);">
			...
			<div style="text-align:center; position: absolute;top: 10%;left: 0%;width: 100%;height: 20%;">
				<p style="font-size: 25px;" class="text-secondary">填写订单信息</p>
			...
			<div style="text-align:center; position: absolute;top:20%;left: 80%;width: 19%;height: 70%;border-color:black; border-width: 1px;border-style: solid;">
				<p style="text-align:center; position: absolute;top: 0%;left: 0%;width: 100%;height: 7%;font-size: 17px;border-bottom-color:black;border-bottom-width:1px;border-bottom-style: solid;">填写收货人信息</p>
				...
				<input id="consigneename" type="text" style="text-align:left; position: absolute;top:40%;left: 5%;width: 90%;height: 10%;font-size: 10px;border-color:black; border-width: 1px;border-style: solid;" class="form-control" placeholder="收货人姓名">
				<input id="consigneesaddress" type="text" style="text-align:left; position: absolute;top:55%;left: 5%;width: 90%;height: 10%;font-size: 10px;border-color:black; border-width: 1px;border-style: solid;" class="form-control" placeholder="收货人地址">
				<input id="consigneephonenumber" type="number" style="text-align:left; position: absolute;top:70%;left: 5%;width: 90%;height: 10%;font-size: 10px;border-color:black; border-width: 1px;border-style: solid;" class="form-control" placeholder="收货人电话">
			</div>
			<!-- 商品清单 -->
			...
			<div id="pdiv" style="position:absolute;text-align:center; left:25%; width:50%;height:10%;top:90%;">
				<button id="backtouserhtml" type="button" class="btn btn-secondary btn-sm" style="position:absolute;left:25%; width:20%;height:50%;top:25%;">返回</button>
				<button id="buy" type="button" class="btn btn-success btn-sm" style="position:absolute;left:55%; width:20%;height:50%;top:25%;">提交</button>
			</div>
...
<script type="text/javascript">
	...
	//初始化名字数组
	<c:forEach var="tempname" items="${name}">
		name.push("${tempname}");
	</c:forEach>
	//初始化类型数组
	<c:forEach var="temptype" items="${type}">
		type.push("${temptype}");
	</c:forEach>
	//初始化价格数组
	<c:forEach var="tempprice" items="${price}">
		price.push("${tempprice}");
	</c:forEach>
	//初始化数量数组
	<c:forEach var="tempenumber" items="${enumber}">
		enumber.push("${tempenumber}");
	</c:forEach>
	...
	//配送费
	let deliveryfee=new Number('${storeInformation.deliveryfee}');
	//加入产品
	for(let i=0;i<n;i++){
...
$(function(){
	...
	$("#buy").click(function(){
		...
		if(consigneename==''|consigneename.length==0|consigneename.length>9){
			alert("***请检查收货人名字***\n收货人名字不应该为空，不应该大于9个字符");
			$("#consigneename").val("");
			return;
		}
		let consigneesaddress=$("#consigneesaddress").val().trim();
		if(consigneesaddress==''|consigneesaddress.length==0|consigneesaddress.length>30|consigneesaddress.length<9){
			alert("***请检查收货人地址***\n收货人地址不应该为空，不应该大于30个字符，不应该少于9个字符");
			return;
		}
		...
		$.post("${pageContext.request.contextPath}/confirmpurchase",
					{
						consigneename:consigneename,
						consigneesaddress:consigneesaddress,
						consigneephonenumber:consigneephonenumber,
						muid:"${muid}",
						productname:name.join("@"),
						producttypes:type.join("@"),
						productprice:price.join("@"),
						expectnumberofproducts:enumber.join("@"),
						deliveryfee:"${storeInformation.deliveryfee}"
					},
